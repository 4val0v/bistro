...
class Theme extends \yii\db\ActiveRecord
{
    ...
    {
        return static::find()->select(['default_sum','bott_col1', 'bott_col2','bott_col3', 'bott_col4' ])->where(['id'=>1])->one();
    }
    public function dataIndexSeo()
    {
        return static::find()->select(['seo_title_main', 'seo_desc_main','seo_keys_main' ])->where(['id'=>1])->one();
    }
    public function dataLayout()
    {
        return static::find()->select(['vk_link','fb_link','site_link','foot_col1','foot_col2','foot_col3','foot_col4' ])->where(['id'=>1])->one();
    }
